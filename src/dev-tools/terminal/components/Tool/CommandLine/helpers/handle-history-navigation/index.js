...
export default ({ event, navigation, setNavigation, setValue, value }) => {
  ...
  if (event.keyCode === keys.UP_ARROW) handleUpArrowKeyPress({ commandHistory, navigation, setNavigation, setValue, value });
  if (event.keyCode === keys.DOWN_ARROW) handleDownArrowKeyPress({ commandHistory, navigation, setNavigation, setValue });
